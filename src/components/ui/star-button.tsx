...
export function StarButton({
    ...
    const effectiveLightColor = lightColor ||
        (variant === 'gold' ? '#F7D979' :
            variant === 'green' ? '#00ef80' :
                variant === 'orange' ? '#ff9f43' :
                    '#FAFAFA');

    const effectiveBackgroundColor = backgroundColor ||
        (variant === 'gold' ? '#F7D979' :
            variant === 'green' ? '#00b894' :
                variant === 'orange' ? '#e17055' :
                    'currentColor');
    ...
    const textGradientClass = variant === 'gold'
        ? "text-black font-black tracking-wide"
        : variant === 'green' || variant === 'orange'
            ? "text-white font-bold tracking-wide drop-shadow-md"
            : "bg-gradient-to-t dark:from-white dark:to-neutral-500 from-black to-neutral-400 text-transparent bg-clip-text";
    ...
    return (
        ...
        >
            ...
            />
            <div
                className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] dark:text-black text-white"
                aria-hidden="true"
            >
                <StarBackground color={effectiveBackgroundColor} />
            </div>
            <span className={`z-10 relative inline-block ${textGradientClass}`}>
                {children}
            </span>
        </button>
    );
